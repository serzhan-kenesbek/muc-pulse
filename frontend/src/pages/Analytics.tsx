...
const EMOTION_CATEGORIES = [
    ...
    {
        label: "Accessible / Inaccessible",
        value: "accessible-inaccessible",
        emotions: ["accessible", "inaccessible"] as EmotionType[],
    },
    { label: "Quiet / Noisy", value: "quiet-noisy", emotions: ["quiet", "noisy"] as EmotionType[] },
    {
        label: "Uncrowded / Crowded",
        value: "uncrowded-crowded",
        emotions: ["uncrowded", "crowded"] as EmotionType[],
    },
...
const Analytics = () => {
    ...
    const filteredSignals = useMemo(() => {
        ...
        // Time-of-day filter
        if (selectedTime !== "all") {
            currentSignals = currentSignals.filter(
                (signal) => signal.timeOfDay === selectedTime
            );
        }
        ...
        if (activeEmotionCategories) {
            ...
            if (category) {
                currentSignals = currentSignals.filter((signal) =>
                    category.emotions.includes(signal.emotion)
                );
            }
        }

        return currentSignals;
    }, [signals, selectedTime, polarityFilter, activeEmotionCategories]);
